...
$stmt = $conn->prepare("SELECT product_name, category_id, brand_id, price, stock, description, status, image FROM products WHERE product_id = ?");
...
if (isset($_POST['update_product'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (in_array($_FILES['image']['type'], $allowed_types)) {
            ...
            if (!$upload_dir) {
                $error = "Uploads folder does not exist. Please create the folder 'uploads'.";
            } else {
                ...
                if (!move_uploaded_file($_FILES['image']['tmp_name'], $upload_path)) {
                    ...
                    if ($product['image'] && file_exists($upload_dir . DIRECTORY_SEPARATOR . $product['image']) && $product['image'] !== $image_name) {
                        unlink($upload_dir . DIRECTORY_SEPARATOR . $product['image']);
                    }
                }
            }
        } else {
    ...
    if (!$error) {
        $stmt = $conn->prepare("UPDATE products SET product_name=?, category_id=?, brand_id=?, price=?, stock=?, description=?, status=?, image=? WHERE product_id=?");
        $stmt->bind_param("siidsissi", $name, $category, $brand, $price, $stock, $description, $status, $image_name, $product_id);
        if ($stmt->execute()) {
...
<head>
  <meta charset="UTF-8" />
  <title>Edit Product - Admin | K.N. Raam Hardware</title>
      ...
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <?php if ($product['image'] && file_exists(__DIR__ . '/../uploads/' . $product['image'])): ?>
            <img src="../uploads/<?= htmlspecialchars($product['image']) ?>" alt="Product Image" style="max-width: 150px; max-height: 150px; border: 1px solid #ccc; padding: 3px;">
          <?php else: ?>
            <em>No image</em>
          <?php endif; ?>
        </div>
