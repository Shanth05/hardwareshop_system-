...
$stmt = $conn->prepare("SELECT category_name, description FROM categories WHERE category_id = ?");
...
if (isset($_POST['update_category'])) {
    ...
    if (!$name) {
        ...
        // Check uniqueness ignoring current category
...
<title>Edit Category - K.N. Raam Hardware Admin</title>
...
<div class="mb-3">
  ...
  <input type="text" name="category_name" class="form-control" value="<?= htmlspecialchars($current_name) ?>" required />
</div>
<div class="mb-3">
  <label class="form-label">Description</label>
  <textarea name="description" class="form-control" rows="3"><?= htmlspecialchars($current_description) ?></textarea>
</div>
